...
## 22. `review-required` 2商品を DRAFT にした(2026-08-26)

ローンチ QA で見つかった未整形の2商品を、**削除せず DRAFT** にした。
`status` 以外は一切触っていない。

| | 手編みコースター | レジン スカルプチャーオブジェ |
|---|---|---|
| GID | `Product/10218057990384` | `Product/10218009166064` |
| handle | `crochet-coaster` | `resin-sculpture-object` |
| status | ACTIVE → **DRAFT** | ACTIVE → **DRAFT** |
| resourcePublications | オンラインストア + POS → **0件** | オンラインストア + POS → **0件** |
| onlineStoreUrl | null | null |
| Variant 数 | **7**(変更なし) | **3**(変更なし) |
| media 数 | **11**(変更なし) | **4**(変更なし) |
| tags | `object` / `review-required` / `tier-aov`(変更なし) | 同左(変更なし) |
| metafield | `custom.multi_buy_eligible = true`(変更なし) | 同左(変更なし) |
| 価格 / SKU | ¥2,980〜¥3,480・7 SKU すべて維持 | ¥3,480・3 SKU すべて維持 |

**`status: ACTIVE` に戻すだけで復帰できる。**

### 「商品情報未整形」の裏づけ

手編みコースターの Variant 名は機械翻訳のまま残っていた:
**「カエルの直径は12cmです」「トカゲ 12 センチ」「明るい色のカメ」「12cm ダック」**。
レジンも「18CM マルセイユ」「15センチデビッド」「15センチメートルアポロ」。
**Owner の QA 判断どおり。**(今回は文言を直さず DRAFT 化のみ)

### new-arrivals での露出

`new-arrivals` は **`VARIANT_PRICE > 0` のスマートコレクション**なので、
**status に関係なく全36商品がルールに一致する。** Admin 上の `productsCount` は 36 のまま。

**ただし DRAFT 商品はどの販売チャネルにも公開されていない**
(8商品すべて `resourcePublications: []`)ので、**ストアフロントには出ない。**

| | |
|---|---|
| new-arrivals ルール一致 | 36 |
| うち DRAFT(顧客に出ない) | **8** |
| **顧客に見える** | **28** |
...
(ソラは Owner 側で DRAFT 済みだった)

### 副次的な影響

- **sale collection は 18 のまま。** 2商品とも `sale.productIds` に入っていないため影響なし
- ⚠️ 手編みコースターは `multi_buy_eligible = true` で、
  **storefront 21ケースの A-2(コースター ×3 → まとめ買い 15%)で使っていた。**
...
## 24. パスワード解除後の実測(2026-08-27)

**ローンチ阻害3件のうち2件は、このセッション開始時点ですでに解消していた。**

### 🔴 正規ドメインが `moruliving.com` に切り替わっていた

| 叩いた URL | 応答 |
|---|---|
| `https://moruliving.com/cart.js` | **200** |
| `https://moruliving.myshopify.com/cart.js` | 301 → `https://moruliving.com/cart.js` |
| `https://rgy5ee-fv.myshopify.com/cart.js` | 301 → `https://moruliving.com/cart.js` |

`run.mjs` は `redirect: 'manual'` で叩くので、**既定が旧ドメインのままだと 301 で即停止する**
(実際に「パスワード保護がまだ有効な可能性がある」と誤検知した)。
...
ストアフロントの疎通(いずれも 200):
`/` / `/collections/all` / `/products/moru-flower-lounge` /
`/pages/privacy` / `/pages/tokushoho` / `/cart` / `/search?q=chair`

### 阻害2: 通常配送の transit time — ✅ 消えている

`/cart/shipping_rates.json` を実際に叩いた(東京 150-0001)。

| カート | 返ってきたレート | `delivery_days` | `delivery_range` |
|---|---|---|---|
| マッシュルーム ×1(¥9,980・しきい値以上) | 通常配送 **¥0** | **`[]`** | **`null`** |
| ミニ アラームクロック ×1(¥4,480・しきい値未満) | 通常配送 **¥870** | **`[]`** | **`null`** |

**「3〜5営業日」は返ってこない。** 送料無料しきい値の切り替わりも実測どおり。
→ Owner 側で管理画面から削除済みと判断してよい。

### 阻害3: MORU500 の顧客への案内 — 🔴 未解消。**引き継ぎの記述より状況が悪い**

コード `MORU500` は**ストアフロント上のどこにも出ていない。**

| ページ | `MORU500` | `500OFF` |
|---|---|---|
| `/` | 0 | 1(アナウンスバー) |
| `/cart` | 0 | 1(アナウンスバー) |
| `/account/register` | **0** | **0** |
| `/account/login` | **0** | **0** |
| `/pages/faq` | 0 | 3 |

**一方で、顧客向け文面は「クーポンがもらえる/表示される」と言っている:**

- アナウンスバー(全ページ): 「¥7,700以上のご購入で送料無料 / **新規会員登録で¥500OFFクーポンプレゼント**」
- FAQ: 「新規会員登録後、**対象のお客様のカートに表示される**¥500OFFクーポンをご利用ください。
  1名様につき1回までご利用いただけます。ほかの割引とは併用できないため、
  現在のカートでは別の割引が優先され、クーポンが適用されない場合があります。」

**FAQ が「カートに表示される」と明言しているのに、その表示ブロックが存在しない。**
= E2E が切れているだけでなく、**顧客向け文面が事実と食い違っている。**

会員登録の導線(`/account/register`)にもコードの記載が無いので、
**新規会員はコードを知る手段が一切ない。**

必要なのは `{% if customer != nil and customer.orders_count == 0 %}` のブロック
(コード提示 + ワンクリック適用)を **`MORU Frontend Dev`(166341181680)** のカートに入れること。
docs/06 §2 の分業ルールにより、**Owner の明示指示があるまで着手しない。**

⚠️ 実装を先送りするなら、**FAQ の「カートに表示される」だけは先に直すべき**
(いま事実に反している)。

### storefront-tests の再実行

`MORU_STORE_URL=https://moruliving.com node ops/promotions/storefront-tests/run.mjs`

- **A-2(手編みコースター ×3)は `cart/add.js` が 422** を返す。
  当該商品を DRAFT にしたため。**想定どおりで、Function の問題ではない**
- **E-2 もコースターを使っている**ので同じ理由で成立しない
- したがって有効なのは 21ケース中 **19ケース**(前回の申し送りは「20ケース」としていたが、
  E-2 を数え漏らしていた)

---

## 24. パスワード解除後の実機テスト(2026-08-27)

**パスワード保護が解除されていたので、storefront 実機テストを実行した。**
`onlineStore.passwordProtection.enabled: false` を確認したうえで実施。

### 🔴 先に環境の変化 — 正規ドメインが変わっていた

`shop.url` は **`https://moruliving.com`**。
`moruliving.myshopify.com` も `rgy5ee-fv.myshopify.com` も **301 でここへ飛ぶ。**

ランナーは `redirect: 'manual'` で叩くので、**旧ドメインを既定にしていると 301 で即停止する。**
実際に1回それで失敗した。既定値を `moruliving.com` に変更した。

### 結果 — **17 PASS / 0 FAIL**

| # | ケース | 小計 | 期待割引 | 実測割引 | 割引後 | 割引名 | 判定 |
|---|---|---|---|---|---|---|---|
| A-1 | フェイクファー 150×200 ×2 | ¥7,960 | -¥796 | -¥796 | ¥7,164 | まとめ買い 10%OFF | ✅ |
| A-2 | 手編みコースター ×3 | — | -¥1,341 | — | — | — | ⏭️ SKIP(DRAFT) |
| A-3 | ジオメトリック 本体 ×1 | ¥7,980 | -¥798 | -¥798 | ¥7,182 | SUMMER SALE 10%OFF | ✅ |
| A-4 | ミニ アラームクロック ×1(しきい値未満) | ¥4,480 | -¥0 | -¥0 | ¥4,480 | — | ✅ |
| A-5 | ボア ×1 | ¥14,980 | -¥1,498 | -¥1,498 | ¥13,482 | SUMMER SALE 10%OFF | ✅ |
| A-6 | ソラ ×1 | — | -¥0 | — | — | — | ⏭️ SKIP(DRAFT) |
| B-1 | クラウド ×1 + マッシュルーム ×1(PAIR) | ¥21,960 | -¥3,294 | -¥3,294 | ¥18,666 | ソファの脇 セット 15%OFF | ✅ |
| B-2 | クラウド ×1 のみ(PAIR 不成立 → Sale) | ¥11,980 | -¥1,198 | -¥1,198 | ¥10,782 | SUMMER SALE 10%OFF | ✅ |
| B-3 | クラウド ×3 + マッシュルーム ×1 | ¥45,920 | -¥5,690 | -¥5,690 | ¥40,230 | ソファの脇 セット 15%OFF / SUMMER SALE 10%OFF | ✅ |
| B-4 | フェイクファー ×2(まとめ買い2点) | ¥5,960 | -¥596 | -¥596 | ¥5,364 | まとめ買い 10%OFF | ✅ |
| B-5 | フェイクファー ×3(まとめ買い3点) | ¥8,940 | -¥1,341 | -¥1,341 | ¥7,599 | まとめ買い 15%OFF | ✅ |
| B-6 | フェイクファー 別Variant 2+1 | ¥8,940 | -¥1,341 | -¥1,341 | ¥7,599 | まとめ買い 15%OFF | ✅ |
| B-7 | フェイクファー ×1 + フランネル ×1(別Product) | ¥5,960 | -¥0 | -¥0 | ¥5,960 | — | ✅ |
| B-8 | フェイクファー ×4 | ¥11,920 | -¥1,788 | -¥1,788 | ¥10,132 | まとめ買い 15%OFF | ✅ |
| C-1 | ハル 1脚 ×1(Sale 対象) | ¥20,980 | -¥2,098 | -¥2,098 | ¥18,882 | SUMMER SALE 10%OFF | ✅ |
| C-2 | ハル 2脚セット ×1(Sale 除外) | ¥33,980 | -¥0 | -¥0 | ¥33,980 | — | ✅ |
| C-3 | ハル 1脚 ×1 + 2脚セット ×1 | ¥54,960 | -¥2,098 | -¥2,098 | ¥52,862 | SUMMER SALE 10%OFF | ✅ |
| D-1 | ソラ ×3 | — | -¥0 | — | — | — | ⏭️ SKIP(DRAFT) |
| D-2 | セル サイドボード ×2 | ¥63,960 | -¥6,396 | -¥6,396 | ¥57,564 | SUMMER SALE 10%OFF | ✅ |
| E-1 | フランネル 150×200 ×2 | ¥7,960 | -¥796 | -¥796 | ¥7,164 | まとめ買い 10%OFF | ✅ |
| E-2 | ミニ ×1 + コースター ×1(しきい値未満) | — | -¥0 | — | — | — | ⏭️ SKIP(DRAFT) |

**SKIP 4件は手編みコースターとソラが DRAFT で `cart/add` が 422 を返すため。**
Function の問題ではない。ACTIVE に戻せばそのまま実行できる。

裏づけられた挙動: まとめ買い 2点10% / 3点以上15%(4点以上も15%) / 別 Variant は合算・
別 Product は合算しない / SUMMER SALE 10% / ハル2脚セットの Sale 除外 /
PAIR 15% と Sale の同時成立 / 送料無料しきい値の境界。

### ランナーを直した(2回)

**1. レート制限。** `/cart/add.js` は IP 単位で 429 を返す。
既存のバックオフ(3s×6)では一度制限に入ると回復できず、**21ケース中14ケースが潰れた。**

- バックオフを **5s→10s→20s→40s→60s→60s→90s** に延長
- **`cart.js` の 429 もリトライ対象に**(従来はここで諦めてケース全体が測定不能になっていた)
- 判定を **PASS / FAIL / SKIP / BLOCKED の4種**に分離。
  従来は DRAFT もレート制限も全部 FAIL に丸められ、
  **「Function が壊れている」と誤読させる出力**になっていた
- `MORU_CASES=B-1,D-2` で失敗分だけ流し直せるように
- BLOCKED が残る限り exit 1(未測定を緑にしない)

**2. 誤った原因報告。** 開始前のプローブが 429 を拾うと
**「パスワード保護がまだ有効な可能性がある」**と出していた。別物なので事実に反する。
429 / 3xx(ドメイン違い) / それ以外 を区別して出すようにした。

### 🔴 実測して分かった transit time の状況(ローンチ阻害 #2)

**「3〜5営業日」は消えている。** `/cart/shipping_rates.json` を2条件で実測:

| カート | レート | `delivery_days` | `delivery_range` |
|---|---|---|---|
| ¥9,980(しきい値以上) | 通常配送 **¥0** | `[]` | `null` |
| ¥4,480(しきい値未満) | 通常配送 **¥870** | `[]` | `null` |

**Owner 側で削除済み。** チェックアウトに営業日表示は出ない。

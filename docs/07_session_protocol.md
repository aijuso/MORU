...
| 2026-08-23 | `docs/brand/reference-sheets/01〜05_*.md` | ブランドビジュアル全般 | **ビジュアルの正(v0.2)。** ⚠️ **画像7枚は未収録** — オーナーがチャットに添付したが実行環境がファイル化できず、テキスト版のみ。README を読むこと |
| 2026-08-23 | (**未受領**) `docs/mockups/home_v3_upper.png` / `home_v3_lower.png` | トップページ | **トップの正(v3)になる予定。** 添付そのものが無かった。`docs/02` v3 はプロンプト本文のみを根拠に書いてある |
| 2026-08-23 | `docs/mockups/home_20260823_full.html` | トップページ全体 | **履歴。** docs/02 v3 に置き換わった。完成形のHTMLだが v0.2 のパレット・6分類・12セクションを反映していない |

...
> 🔴 **2026-09-03 方針転換(Owner)**: MORU はやめる方向。Meta広告は配信3日目で停止済み。
> このリポジトリの仕組みを汎用化した **`aijuso/brand-template`** を作成・v1.0 push 済み
> (リサーチ〜広告〜商品登録〜テーマのフルスタック。経緯は `docs/template_plan_20260903.md`)。
> 今後の新ブランドはテンプレートから新リポジトリを生成する。
> このリポジトリは MORU の記録としてアーカイブ予定(正式決定は未起票)。
> 未処理の後始末: 注文#1001 の履行確認 / ストアの休止判断 / 価格改定案(承認待ちのまま)の取り下げ判断。
...
> 🔌 **Winning Hunter 接続済み(2026-08-31 実測)。** claude.ai カスタムコネクタ(OAuth)経由で
> `mcp__WinningHunter__*` が使える。クレジット 98/100 残・**1ツール呼び出し=1クレジット**。
> 手順とフォールバックは `ops/research/winning_hunter_setup.md`。

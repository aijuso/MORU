...
16. **商品リサーチを行うときは、着手前に `ops/research/rubric.md` を読むこと。** 評価基準はそこが正
17. **広告クリエイティブの制作・競合広告の分析を行うときは、着手前に
    `.claude/skills/ad-benchmark-creative/` を読むこと。** ワークフロー・AdWhispr の使い方・
    「借りる/捨てる」の線引きはそこが正。分析結果の保存先は `ops/ads/benchmarks/`(取得日必須)
